    ...
    return (
        <div className={styles.main}>
            ...
            <div className={styles.data_container}>
                {data.map(item => (
                    <div className={styles.data_content} key={item.id}>
                        <p className={`${styles.data_text} ${blur ? styles.data_text_blur : ""}`} key={item.id}>
                            {item.value}
                        </p>
                        <svg className={styles.svg} onClick={() => handleDelete(item.id)} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
